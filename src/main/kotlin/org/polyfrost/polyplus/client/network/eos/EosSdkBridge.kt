...
class EosSdkBridge {
    ...
    @Volatile private var inboundHandler: ((Received) -> Unit)? = null
    @Volatile private var loginLostHandler: (() -> Unit)? = null

    private data class StateHandlerEntry(val handle: Long, val callback: (ConnectionStateEvent) -> Unit)
    ...
    @Volatile var localUser: EosProductUserId? = null
        private set
    ...
    private fun dispatchState(
        ...
        make: (EosProductUserId) -> ConnectionStateEvent,
    ) {
    ...
    private fun checkCongestion() {
        P2PChannelRegistry.allChannels().forEach { channel ->
            ...
            channel.updateCongestion(outboundQueueBytes() >= CONGESTION_THRESHOLD_BYTES)
        }
    }
    ...
    fun setRelayControl(forceRelays: Boolean) = post {
    ...
    fun sendPacket(
        ...
        val sdkReliability = when (reliability) {
            PacketReliability.UnreliableUnordered -> EosPacketReliability.UnreliableUnordered
            PacketReliability.ReliableUnordered -> EosPacketReliability.ReliableUnordered
    ...
    fun addConnectionRequestHandler(socket: EosP2PSocketId, handler: (remote: EosProductUserId) -> Unit) {
        requestHandlers[socket.name] = handler
    }

    fun acceptConnection(socket: EosP2PSocketId, remote: EosProductUserId) = post {
        val local = localUser
        if (local == null) {
